...
default check_support build library mw_wrappers tools env_val java_api csharp_api nodejs_api perl_api python3_api ruby_api go_api patch-mac api_docs clean install:
...
default: check_support build library mw_wrappers tools env_val java_api csharp_api nodejs_api perl_api python3_api ruby_api go_api patch-mac
...
ifneq (, $(shell which go))
	...
	@echo "#"
	@echo "#  go not found"
	@echo "#  Skipping build of Go binding for GMSEC API 5.x"
	@echo "#"
	@echo "###########################################################"
endif
else
...
ifneq (, $(shell which perl))
	...
	@ for dir in framework examples wrapper java csharp/gmsec5 nodejs/gmsec5 perl/gmsec5 python3/gmsec5 go/gmsec5 doxygen tests tools; do \
		$(MAKE) -C $$dir $@ ; \
	done
	@ cd ruby/gmsec5 && $(MAKE) -f MakeRuby $@
	$(RM) $(BINDIR)/*
